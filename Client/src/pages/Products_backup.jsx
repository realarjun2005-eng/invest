          ...
          {(Array.isArray(plans) ? plans.filter(p => (p.planType || 'daily') === planType) : []).map((plan, idx) => (
            <div key={idx} className="bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden hover:shadow-xl transition-shadow duration-300">
              <div className="flex flex-col lg:flex-row">
                {/* Image Section */}
                <div className="lg:w-1/2 bg-gradient-to-br from-slate-50 to-gray-100 p-6 relative">
                  <img
                    src={getProductImage(plan.image)}
                    alt={plan.title}
                    className="w-full h-56 lg:h-64 object-cover rounded-xl shadow-lg border border-gray-200"
                    onError={e => { e.target.onerror = null; e.target.src = reactLogo; }}
                  />
                  {plan.offer && (
                    <div className="absolute top-8 right-8 bg-gradient-to-r from-orange-500 to-red-500 text-white px-4 py-2 rounded-full text-sm font-semibold shadow-lg">
                      {plan.offer}
                    </div>
                  )}
                </div>
